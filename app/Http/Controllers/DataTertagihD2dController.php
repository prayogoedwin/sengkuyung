...
class DataTertagihD2dController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $years = ApiCacheManager::remember('admin:data-tertagih-d2d:years', ApiCacheManager::dataTtl(), static function () {
            return DataTertagihD2d::select('year')
                ->distinct()
                ->orderBy('year', 'desc')
                ->pluck('year')
                ->toArray();
    ...
    public function downloadTemplate(string $format, string $type)
    {
        ...
        $filename = $isExample
            ? 'data-tertagih-d2d-contoh-10-row.' . $format
            : 'data-tertagih-d2d-format-kosong.' . $format;
    ...
    public function updateStatus(Request $request, int $id)
    {
        ...
        $data = DataTertagihD2d::findOrFail($id);
