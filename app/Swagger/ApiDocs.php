...
class ApiDocs
{
    ...
    #[OA\Post(
        ...
        responses: [
            ...
            new OA\Response(response: 401, description: 'Unauthorized'),
            new OA\Response(response: 403, description: 'Bukan role petugas'),
            new OA\Response(response: 422, description: 'Validasi gagal'),
        ]
    )]
    ...
    #[OA\Get(
        ...
        description: '**Autentikasi:** Wajib Bearer token di header `Authorization: Bearer {token}`. Token didapat dari `POST /api/login-external` (klik Authorize di Swagger untuk mengisi token). Filter mengikuti halaman dapur/verifikasi. Default status = bucket menunggu verifikasi jika `status_verifikasi_id` kosong. Kirim `status_verifikasi_id=15` untuk semua status. Pencarian `nopol` mengabaikan filter lain.',
        security: [['bearerAuth' => []]],
        parameters: [
            new OA\Parameter(name: 'status_verifikasi_id', in: 'query', required: false, schema: new OA\Schema(type: 'integer', example: 1), description: 'Filter status verifikasi. Kosong = bucket menunggu. Nilai `15` = semua status (tanpa filter status).'),
            ...
            new OA\Parameter(name: 'tanggal_start', in: 'query', required: false, schema: new OA\Schema(type: 'string', format: 'date', example: '2026-01-01'), description: 'Filter tanggal pendataan (`created_at`), wajib dipasangkan dengan `tanggal_end`'),
            new OA\Parameter(name: 'tanggal_end', in: 'query', required: false, schema: new OA\Schema(type: 'string', format: 'date', example: '2026-12-31'), description: 'Filter tanggal pendataan (`created_at`), wajib dipasangkan dengan `tanggal_start`'),
            new OA\Parameter(name: 'tanggal_update_start', in: 'query', required: false, schema: new OA\Schema(type: 'string', format: 'date', example: '2026-01-01'), description: 'Filter tanggal update (`updated_at`), wajib dipasangkan dengan `tanggal_update_end`'),
            new OA\Parameter(name: 'tanggal_update_end', in: 'query', required: false, schema: new OA\Schema(type: 'string', format: 'date', example: '2026-12-31'), description: 'Filter tanggal update (`updated_at`), wajib dipasangkan dengan `tanggal_update_start`'),
            new OA\Parameter(name: 'page', in: 'query', required: false, schema: new OA\Schema(type: 'integer', default: 1)),
            new OA\Parameter(name: 'per_page', in: 'query', required: false, schema: new OA\Schema(type: 'integer', default: 10)),
        ],
        responses: [
    ...
    #[OA\Get(
        ...
        description: '**Autentikasi:** Wajib Bearer token di header `Authorization: Bearer {token}`. Token didapat dari `POST /api/login-external` (klik Authorize di Swagger untuk mengisi token). Filter mengikuti halaman dapur/verifikasi-d2d. Gunakan `/api/master/status-verifikasi` untuk daftar status. Kirim `status_verifikasi_id=15` untuk semua status.',
        security: [['bearerAuth' => []]],
        parameters: [
            new OA\Parameter(name: 'status_verifikasi_id', in: 'query', required: false, schema: new OA\Schema(type: 'integer', example: 1), description: 'Filter status verifikasi. Kosong = bucket menunggu. Nilai `15` = semua status (tanpa filter status).'),
