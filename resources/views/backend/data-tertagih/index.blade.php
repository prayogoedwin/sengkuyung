...
@push('js')
    <script>
        $(document).ready(function() {
            ...
            const importUploadUrl = @json(route('data-tertagih.import.upload'));
            ...
            $('#importForm').on('submit', async function(e) {
                ...
                try {
                    ...
                    let done = false;
                    while (!done) {
                        $progress.text('Memproses data... ' + (uploadData.stats?.inserted ?? ''));
        ...
        function forceDeleteTertagih(id) {
            Swal.fire({
                title: 'Force Delete?',
                html: 'Data akan diarsipkan ke tabel <code>_del</code>, lalu dihapus dari data tertagih dan pendataan terkait (nopol sama).<br><b>Hanya super admin.</b>',
                icon: 'warning',
                showCancelButton: true,
                confirmButtonColor: '#d33',
                cancelButtonColor: '#3085d6',
                ...
                $.ajax({
                    url: "{{ route('data-tertagih.force-destroy', ':id') }}".replace(':id', id),
                    type: 'DELETE',
                    data: {
                        _token: '{{ csrf_token() }}'
                    },
