...
<script>
    function forceDeleteVerifikasi(id) {
        Swal.fire({
            title: 'Force Delete?',
            html: 'Data akan diarsipkan ke tabel <code>_del</code>, lalu dihapus dari pendataan dan data tertagih terkait (nopol sama).<br><b>Hanya super admin.</b>',
            icon: 'warning',
            ...
            $.ajax({
                url: "{{ route($verifikasiForceDestroyRoute ?? 'verifikasi.force-destroy', ':id') }}".replace(':id', id),
                type: 'DELETE',
                data: {
                    _token: '{{ csrf_token() }}'
                },
